function [CytoSingleRNAMask,GauFil] = SinglePlaneRNAMaskLocalMax_cjyRemoveNucFirst(SinglemRNA,SinglePlaneNuc,ExRNACore,IxRNACore,IndexOfZstack,mRNAFociRawMaskPlane)
%%
% filcore = fspecial('average',3);
% filSinglemRNA = imfilter(SinglemRNA,filcore,'replicate');
% % % RemoveRNA1 = SinglePlaneNuc;
% % %  RemoveRNA2 = mRNAFociRawMaskPlane;
% % % SinglemRNARemoveNuc = SinglemRNA.*(~RemoveRNA1);
% % % SinglemRNARemoveNuc = SinglemRNARemoveNuc.*(~RemoveRNA2);
RemoveRNA = SinglePlaneNuc | mRNAFociRawMaskPlane;
SinglemRNARemoveNuc = SinglemRNA.*(~RemoveRNA);
% figure();
% imshow(SinglemRNARemoveNuc(512:1024,512:1024),[]);


Ex = fspecial('gaussian',21,ExRNACore);%10
Ix = fspecial('gaussian',21,IxRNACore);%20
outE = imfilter(single(SinglemRNARemoveNuc),Ex,'replicate'); 
outI = imfilter(single(SinglemRNARemoveNuc),Ix,'replicate'); 
outims = outE - outI;  
...
GauFil = imgaussfilt(single(SinglemRNARemoveNuc),1.5);
% figure();
% imshow(AdjOutims(512:1024,512:1024));
