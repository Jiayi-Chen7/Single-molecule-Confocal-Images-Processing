function [CytoSingleRNAMask,GauFil,SinglemRNANor] = SinglePlaneRNAMaskLocalMax_Nor_cjy(SinglemRNA,SinglePlaneNuc,ExRNACore,IxRNACore,IndexOfZstack,mRNAFociRawMaskPlane)
%%
% filcore = fspecial('average',3);
% filSinglemRNA = imfilter(SinglemRNA,filcore,'replicate');
NucArea = SinglePlaneNuc |  mRNAFociRawMaskPlane;
SinglemRNA = SinglemRNA.*(~NucArea);
SinglemRNANor = ((SinglemRNA - min(min(SinglemRNA )))./(max(max(SinglemRNA)) - min(min(SinglemRNA)))).*65535;
Ex = fspecial('gaussian',21,ExRNACore);%10
Ix = fspecial('gaussian',21,IxRNACore);%20
outE = imfilter(single(SinglemRNANor),Ex,'replicate'); 
outI = imfilter(single(SinglemRNANor),Ix,'replicate'); 
outims = outE - outI;  
...
GauFil = imgaussfilt(single(SinglemRNANor),1.5);


NumOfCanRNAMask = zeros(1,200);
ThVal = linspace(0,1,200);
for i = 1:200
    CanRawRNAMask = imbinarize(Gauoutims,ThVal(i));
...
RawRNAMask = imbinarize(Gauoutims,ThVal(ElbowPointIndexRelative));
...
% class(CytoRNAMask)
% class(AdjOutims)
OutimsAfterMask = double(Gauoutims).*CytoRNAMask;
