function [CytoSingleGFPMask,GauGFPFil] = SinglePlaneGFPMask2(SingleGFP,SinglePlaneNuc,ExGFPCore,IxGFPCore,IndexOfZstack,GauKer)
%%
% filcore = fspecial('average',3);
% filSinglemRNA = imfilter(SinglemRNA,filcore,'replicate');
SingleGFPInput = SingleGFP;  
RemoveRegion = SinglePlaneNuc;
SingleGFP = double(SingleGFP).*(~RemoveRegion);
...
ElbowPointIndexRelative = GetElbowIndex(NumOfCanGFPMask(1:end-1));
...
% LLCytoGFPMask = bwlabel(CytoGFPMask);
% stsCytoGFPMask = regionprops(LLCytoGFPMask,SingleGFPInput,'Area','MaxIntensity');
% GFPFociArea = [stsCytoGFPMask.Area];
% GFPFociMaxInt = [stsCytoGFPMask.MaxIntensity];
% figure();
% histogram(GFPFociArea);
% figure();
% histogram(GFPFociMaxInt);
% OutimsAfterMask = double(SingleGFP).*CytoGFPMask;
% 
% RawGFPMask2 = imregionalmax(OutimsAfterMask);
...
% CytoSingleRNAMask = CytoRNAMask;(1536:2048,1536:2048)
%plot
% % % % LowHighRNA2 = stretchlim(uint16(SingleGFP));
% % % % AdjOutims2 = imadjust(uint16(SingleGFP),[0 max(LowHighRNA2)+0.1],[]);
% % % % figure();
% % % % imshow(AdjOutims2,[]);
% % % % hold on
