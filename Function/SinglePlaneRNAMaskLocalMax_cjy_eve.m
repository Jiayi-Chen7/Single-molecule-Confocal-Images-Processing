function [CytoSingleRNAMask,GauFil] = SinglePlaneRNAMaskLocalMax_cjy_eve(SinglemRNA,SinglePlaneNuc,ExRNACore,IxRNACore,IndexOfZstack,mRNAFociRawMaskPlane)
%%
...
% % % end
 [T EM] = graythresh(AdjOutims);  
    RawRNAMask = imbinarize(AdjOutims,T);
...
% % % imshow(SinglemRNA,[]);
% % % hold on
% % % [B,L] = bwboundaries(CytoRNAMask);
% % % [BN,LN] = bwboundaries(SinglePlaneNuc);
% % % [BR,LR] = bwboundaries(mRNAFociRawMaskPlane);
% % % for k = 1:length(B)
% % %    boundary = B{k};
