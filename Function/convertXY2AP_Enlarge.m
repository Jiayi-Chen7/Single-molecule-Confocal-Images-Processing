function [ventralData, dorsalData, APlength, AP_x, AP_y,NormAPn] = convertXY2AP_Enlarge(intens, nx, ny, emmask, fig,flipAP) %(NucleiIntensity, NucleiPosition(:,1), NucleiPosition(:,2), embryomask, fig)
...
% %     
   ...
   % ventralData(:,1) = ventralData(:,1) / max(ventralData(:,1));
    ...
    if fig
        hold on;
%         plot(nx(ventralPts), ny(ventralPts), 'b.','MarkerSize',30);
        plot(nx, ny, 'b.','MarkerSize',30);

        line(AP_x, AP_y, 'Color',[.8 .8 .0],'LineWidth',5);
        ...
        p3 = p - repmat(n, s+1, 1) * 70;
        ii = line([p1(:,1) p2(:,1)]', [p1(:,2) p2(:,2)]', 'lineStyle','--', 'Color',[.8 .8 0],'LineWidth',5);
        
        for i=1:s+1
            percent = int2str(100/s*(s+1-i));
            text(p3(i,1),p3(i,2),['\color{magenta}', percent, '% EL'], 'horizontalAlignment', 'right','FontSize',20);
        end
       xlim([min(nx),max(nx)]);
       ylim([min(ny),max(ny)]);
        hold off;
