...
ind = find(MatrixOfRNASpot(:,3) < zstepRange(i+1) & MatrixOfRNASpot(:,3) >=zstepRange(i));
RNACountAtEachZ(i) = sum(MatrixOfRNASpot(ind,5)); 
ind2 = find(OnMatrixOfRNASpot(:,3) < zstepRange(i+1) & OnMatrixOfRNASpot(:,3) >=zstepRange(i));
...
for j = 1:length(ErodeNucWeiCen)
    RNA2NucIndPerZ = find(MatrixOfRNASpot(:,6) == j & MatrixOfRNASpot(:,3) < zstepRange(i+1) & MatrixOfRNASpot(:,3) >=zstepRange(i));
    RNANumPerZPerNuc(i,j) = sum(MatrixOfRNASpot(RNA2NucIndPerZ,5));
    OnRNA2NucIndPerZ = find(OnMatrixOfRNASpot(:,6) == j & OnMatrixOfRNASpot(:,3) < zstepRange(i+1) & OnMatrixOfRNASpot(:,3) >=zstepRange(i));
...
clear i
for i = 1:(length(zstepRange)-1)   
ind = find(ColGFPMatrix(:,3) < zstepRange(i+1) & ColGFPMatrix(:,3) >=zstepRange(i));
ColGFPIntAtEachZ(i) = sum(ColGFPMatrix(ind,4)); 
end
%RNATEAtEachZ
clear i
for i = 1:(length(zstepRange)-1)   
ind = find(SingleRNATETot(:,3) < zstepRange(i+1) & SingleRNATETot(:,3) >=zstepRange(i));
RNATEAtEachZ(i) = sum(SingleRNATETot(ind,6)); 
end
%GFPTEAtEachZ
clear i
for i = 1:(length(zstepRange)-1)   
ind = find(SingleGFPTETot(:,3) < zstepRange(i+1) & SingleGFPTETot(:,3) >=zstepRange(i));
